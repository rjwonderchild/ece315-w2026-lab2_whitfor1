...
int main(void)
{
  ...
  // From here and below all of the xil_printf statements have been replaced with print_string.
...
void receive_string(char *buf, size_t buf_len)
{
    ...
    while (1) {
            ...
            if (idx < buf_len -1) {
                buf[idx++] = recvd;
                // The following keeps the string null-terminated.
                buf[idx] = '\0'; 
            } 
            /*
            else {
                buf[buf_len - 1] = '\0';
                return;
            } */ // This part has been removed as if the string is full, then we need to ignore any additional characters that come in.
        } else {
            vTaskDelay(pdMS_TO_TICKS(POLL_DELAY_MS));
        }
    }
...
void print_string(const char *str) {
  // Need to check if that we do not have a pointer to null
  if (str == NULL) {
    return;
  }
